...
uint16_t Bulk_Data_Len;
Bulk_Only_CBW CBW;
Bulk_Only_CSW CSW;

static uint32_t SCSI_LBA;
static uint32_t SCSI_BlkLen;
...
void Mass_Storage_Out(void)
{
    ...
    Bulk_Data_Len = USB_SIL_Read(EP2_OUT, Bulk_Data_Buff);
...
void CBW_Decode(void)
{
    ...
    for (Counter = 0; Counter < Bulk_Data_Len; Counter++) {
        *((uint8_t *)&CBW + Counter) = Bulk_Data_Buff[Counter];
    }
    CSW.dTag = CBW.dTag;
    CSW.dDataResidue = CBW.dDataLength;
    if (Bulk_Data_Len != BOT_CBW_PACKET_LENGTH) {
        Bot_Abort(BOTH_DIR);
